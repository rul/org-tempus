  ...
  (org-tempus-test--with-clocked-suspend-state
    (let ((now (seconds-to-time 1060))
      ...
      (cl-letf (((symbol-function 'current-time) (lambda () now))
        ...
        (should (equal org-tempus--auto-clock-out-time clock-out-at))
        (should-not org-tempus--session-start-time)))))

(ert-deftest org-tempus-expired-resume-window-falls-back-to-default-task ()
  (let ((now (seconds-to-time 87400))
        (clock-active nil)
        (clocked-in-to nil)
        (org-tempus-auto-clock-enabled t)
        (org-tempus-auto-clock-in-last t)
        (org-tempus-auto-clock-in-window-minutes 120)
        (org-tempus-auto-clock-default-task-id "default")
        (org-tempus--auto-clock-out-time (seconds-to-time 1000))
        (org-tempus-notifications-enabled nil))
    (cl-letf (((symbol-function 'current-time) (lambda () now))
              ((symbol-function 'org-clock-is-active) (lambda () clock-active))
              ((symbol-function 'org-tempus--clock-in-default)
               (lambda (_start-time)
                 (setq clocked-in-to 'default)
                 (setq clock-active t)
                 t))
              ((symbol-function 'org-tempus--update-mode-line) #'ignore))
      (should-not (org-tempus--maybe-auto-clock-in))
      (should (org-tempus--maybe-auto-clock-in-default))
      (should (eq clocked-in-to 'default)))))
